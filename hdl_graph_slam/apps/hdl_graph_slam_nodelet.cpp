...
public:
  ...
  virtual void onInit() {
    ...
    path_pub = mt_nh.advertise<nav_msgs::Path>("/hdl_graph_slam/odom", 1);
    //check_pub = mt_nh.advertise<Eigen::Isometry3d>("/hdl_graph_slam/check", 1);
...
private:
  ...
  void map_points_publish_timer_callback(const ros::WallTimerEvent& event) {
    ...
    //tf::poseEigenToMsg	(	&snapshot.back()->pose, posel);
    ...
    auto arr = cloud->points.size();
    
    //std::cout << "================================================" << std::endl;
     //std::cout<<"The length of the given Array is: " << typeid(snapshot.back()->pose).name() << std::endl;


    nav_msgs::Path odom_x;
    /*odom_y.header.seq = cloud_msg->header.seq;
    odom_y.header.frame_id = "map";

    geometry_msgs::PoseStamped whisker_pose;
    whisker_pose.pose= pose;
    whisker_pose.header = cloud_msg->header;
    whisker_pose.header.frame_id = "map";
    odom_y.poses.push_back(whisker_pose);*/

    odom_x.header.seq = cloud_msg->header.seq;
    odom_x.header.frame_id = "map";
    ...
    for(int i =0; i<cloud->points.size(); i += 1)
    {
      ...
      if ((i+1) % 32 == 0)
      {
        sum_x = sum_x / 32;
        sum_y = sum_y / 32;
        whisker_pose.pose.position.x = sum_x;
        whisker_pose.pose.position.y = sum_y;
        whisker_pose.pose.position.z = 0.0;
        whisker_pose.pose.orientation.x = 0.0;
        whisker_pose.pose.orientation.y = 0.0;
        whisker_pose.pose.orientation.z = 0.0;
        whisker_pose.pose.orientation.w = 0.1;
        whisker_pose.header = cloud_msg->header;
        whisker_pose.header.frame_id = "map";
        odom_x.poses.push_back(whisker_pose);
        sum_x = 0;
        sum_y = 0;
      }
    }
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    if(odom2map_pub.getNumSubscribers()) {
      ...
      /*odom_y.header = ts.header;
    odom_y.header.frame_id = "map";
      geometry_msgs::PoseStamped whisker_pose;
    whisker_pose.pose.position.x = ts.transform.translation.x;
    whisker_pose.pose.position.y = ts.transform.translation.y;
    whisker_pose.pose.position.z = 0.0;
    whisker_pose.pose.orientation.x = 0.1;
    whisker_pose.pose.orientation.y = 0.1;
    whisker_pose.pose.orientation.z = 0.1;
    whisker_pose.pose.orientation.w = 0.1;
    whisker_pose.header = ts.header;
    whisker_pose.header.frame_id = "map";
    odom_y.poses.push_back(whisker_pose);
    

    std::cout << "================================================" << std::endl;*/
    //path_pub.publish(odom_y);
...
private:
  ...
  geometry_msgs::PoseStamped whisker_pose_init;
  bool INIT;

  nav_msgs::Path odom_y;
